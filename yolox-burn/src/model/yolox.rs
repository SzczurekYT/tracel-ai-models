...
impl<B: Backend> Yolox<B> {
    ...
    pub fn yolox_nano_pretrained(
        ...
        model.load_weights_from_store(&weights, device)?;
    ...
    /// Load specified pre-trained PyTorch weights from store.
    #[cfg(feature = "pretrained")]
    fn load_weights_from_store(
        &mut self,
        weights: &weights::Weights,
        device: &Device<B>,
    ) -> Result<(), PytorchStoreError> {
        // Download torch weights
        let weights_path = weights.download().map_err(|err| {
        ...
        //     .with_key_remapping(r"backbone\.C3_(.+)", "backbone.c3_$1")
        //     // Map backbone.backbone.dark[i].0.* -> backbone.backbone.dark[i].conv.*
        //     .with_key_remapping(r"(backbone\.backbone\.dark[2-5])\.0\.(.+)", "$1.conv.$2")
        //     // Map backbone.backbone.dark[i].1.* -> backbone.backbone.dark[i].c3.*
        //     .with_key_remapping(r"(backbone\.backbone\.dark[2-4])\.1\.(.+)", "$1.c3.$2")
        //     // Map backbone.backbone.dark5.1.* -> backbone.backbone.dark5.spp.*
        //     .with_key_remapping(r"(backbone\.backbone\.dark5)\.1\.(.+)", "$1.spp.$2")
        //     // Map backbone.backbone.dark5.2.* -> backbone.backbone.dark5.c3.*
        //     .with_key_remapping(r"(backbone\.backbone\.dark5)\.2\.(.+)", "$1.c3.$2")
        ...
        // if let Some(ref mut spp) = self.backbone.backbone.dark5.spp {
